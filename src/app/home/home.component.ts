import { Component, computed, OnInit } from '@angular/core';
import { RouterModule } from '@angular/router';
import { Category, IProduct, Product } from '../Models/product';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { CommonFunctionService } from '../Services/common-function.service';
import { TestService } from '../Services/test.service';
import { ErrorDialogComponent } from "../error-dialog/error-dialog.component";
import { ErrorService } from '../Services/error.service';
import { ApiService } from '../Services/api.service';


@Component({
  selector: 'app-home',
  imports: [RouterModule, CommonModule, FormsModule, ErrorDialogComponent],
  templateUrl: './home.component.html',
  styleUrl: './home.component.scss'
})
export class HomeComponent implements OnInit{

  constructor(private commonFunc : CommonFunctionService,
            private test : TestService,
             private err : ErrorService,
             private api : ApiService
            ){
              debugger
         this.testForHome = computed(() => this.test.testVar() )
         this.showErr = computed(() => this.err.showErrOrNot() )
  }

  ngOnInit(){
    debugger
     this.api.getData('https://restaurant.stepprojects.ge/api/Products/GetAl')
        .subscribe(data => console.log(data))
  }

  showErr! :any

  testForHome! : any

  showCards = false;
  btnTitle = "Sort Asc"
  name = "saxeli"
  search = ""
  filteredArr  : Product[] = []
  isFiltered = false

  width = "200"
  isDisabled = false
 fun(){
  console.log("hi")
 }

//   productArr : Product[] = [
//   {
//     "id": 1,
//     "title": "Fjallraven - Foldsack No. 1 Backpack, Fits 15 Laptops",
//     "price": 109.95,
//     "description": "Your perfect pack for everyday use and walks in the forest. Stash your laptop (up to 15 inches) in the padded sleeve, your everyday",
//     "category": Category.man,
//     "image": "https://fakestoreapi.com/img/81fPKd-2AYL._AC_SL1500_t.png",
//     "rating":  3.9,
//   },
//   {
//     "id": 2,
//     "title": "Mens Casual Premium Slim Fit T-Shirts ",
//     "price": 22.3,
//     "description": "Slim-fitting style, contrast raglan long sleeve, three-button henley placket, light weight & soft fabric for breathable and comfortable wearing. And Solid stitched shirts with round neck made for durability and a great fit for casual fashion wear and diehard baseball fans. The Henley style round neckline includes a three-button placket.",
//     "category": Category.man,
//     "image": "https://fakestoreapi.com/img/71-3HjGNDUL._AC_SY879._SX._UX._SY._UY_t.png",
//     "rating":  4.1,
//   },
//   {
//     "id": 3,
//     "title": "Mens Cotton Jacket",
//     "price": 55.99,
//     "description": "great outerwear jackets for Spring/Autumn/Winter, suitable for many occasions, such as working, hiking, camping, mountain/rock climbing, cycling, traveling or other outdoors. Good gift choice for you or your family member. A warm hearted love to Father, husband or son in this thanksgiving or Christmas Day.",
//     "category":Category.woman,
//     "image": "https://fakestoreapi.com/img/71li-ujtlUL._AC_UX679_t.png",
//     "rating": 4.7
    
//   },
//   {
//     "id": 4,
//     "title": "Mens Casual Slim Fit",
//     "price": 15.99,
//     "description": "The color could be slightly different between on the screen and in practice. / Please note that body builds vary by person, therefore, detailed size information should be reviewed below on the product description.",
//     "category": Category.woman,
//     "image": "https://fakestoreapi.com/img/71YXzeOuslL._AC_UY879_t.png",
//     "rating": 430
    
//   },
//   {
//     "id": 5,
//     "title": "John Hardy Women's Legends Naga Gold & Silver Dragon Station Chain Bracelet",
//     "price": 695,
//     "description": "From our Legends Collection, the Naga was inspired by the mythical water dragon that protects the ocean's pearl. Wear facing inward to be bestowed with love and abundance, or outward for protection.",
//     "category": Category.man,
//     "image": "https://fakestoreapi.com/img/71pWzhdJNwL._AC_UL640_QL65_ML3_t.png",
//     "rating":  400
//   }
  
// ]


show(){
   this.showCards ? this.showCards =false :  this.showCards = true
   this.commonFunc.print()

}

// sort(){

//     this.btnTitle == "Sort Asc" ?  this.btnTitle = "Sort Desc" :   this.btnTitle = "Sort Asc"
//     this.btnTitle == "Sort Asc" 
//     ? this.productArr = this.productArr.sort((a,b) =>  b.price - a.price) 
//     : this.productArr = this.productArr.sort((a,b) => a.price - b.price) 

// }

resetFilter(){
  this.isFiltered = false;
     this.filteredArr = []
     this.search = "";

    //  this.pordcutX.consverTitle()
}
// searchByTitle(){
//    this.isFiltered =true;
//    this.filteredArr = this.productArr.filter(el => el.title.includes(this.search))
// }


   pordcutX = new Product







}



